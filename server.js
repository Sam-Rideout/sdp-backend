    ...
    return new Promise((resolve, reject) => {
        execFile(
            ...
            (error, stdout, stderr) => {
                ...
                const maxMatch = output.match(/max_volume:\s*(-?\d+(\.\d+)?) dB/i);
                const meanMatch = output.match(/mean_volume:\s*(-?\d+(\.\d+)?) dB/i);

                if (!maxMatch) {
                    reject(new Error('Could not analyze recording volume.'));
                    return;
                }
...
function validateRecordingQuality(duration, volumeInfo) {
    if (duration < MIN_NAME_SECONDS) {
        throw new Error('Recording is too short. Please sing the name clearly.');
    }

    if (duration > MAX_NAME_SECONDS) {
        throw new Error('Recording is too long. Please sing only the first name.');
    }

    if (volumeInfo.maxVolumeDb < TOO_QUIET_DB) {
        throw new Error('Recording is too quiet. Please sing the name a little closer to the microphone.');
    }

    if (volumeInfo.maxVolumeDb > CLIPPED_DB) {
        throw new Error('Recording is too loud or distorted. Please sing a little softer or farther from the microphone.');
    }
}
...
app.post('/upload', upload.single('audio'), async (req, res) => {
    ...
    try {
        ...
        const volumeInfo = await analyzeAudioVolume(cleanWavPath);

        console.log('Clean name duration:', nameDuration);
        console.log('Volume analysis:', volumeInfo);

        validateRecordingQuality(nameDuration, volumeInfo);
